...
export default class RGB extends React.Component<{}, RGBState> {
  ...
  // 处理变换
  rgbConvert = (en: boolean) => {
    const { red,green,blue,hex } = this.state
    let result = ""
    if (en) {
      result = this.rgbToHex(red,green,blue)
    }else{
  ...
  rgbToHex = (red:string, greeb:string, blue:string):string => {
    let hexcode = "#";
    let rbgs = [red, greeb, blue];
    for (let x = 0; x < 3; x++) {
      var n = parseInt(rbgs[x]);
  ...
  handleInputChange = (key: string, value: string) => {
    this.setState({
      [key]: value,
    } as Pick<RGBState, keyof RGBState>);
  };

  render() {
    const { hex, red, green, blue,result } = this.state;
    return (
      <Layout>
        <div className="container rgb">
          <div className="transfer content">
            <div>
              <h5 className="rgb-t">RGB颜色值转换成十六进制颜色码：</h5>
              <div className="columns is-2">
                <div className="column">
                  <input
                    ...
                    type="text"
                    onChange={(e) =>
                      this.handleInputChange("red", e.target.value)
                    }
                  />
                </div>
                <div className="column">
                  <input
                    ...
                    type="text"
                    onChange={(e) =>
                      this.handleInputChange("green", e.target.value)
                    }
                  />
                </div>
                <div className="column">
                  <input
                    ...
                    type="text"
                    onChange={(e) =>
                      this.handleInputChange("blue", e.target.value)
                    }
                  />
                </div>
              </div>
              <button
                ...
                onClick={() => this.rgbConvert(true)}
              >
                转换
              </button>
            </div>
            <div>
              <h5 className="rgb-t">十六进制颜色码转换成RGB颜色值：</h5>
              <input
                className="input is-small mb10"
                value={hex}
                type="text"
                onChange={(e) => this.handleInputChange("hex", e.target.value)}
              />
              <button
                className="button is-small is-primary"
                onClick={() => this.rgbConvert(false)}
              >
                转换
              </button>
            </div>
            <div>
              {
                result.length>0?(<p>{result}</p>):null
              }
            </div>
          </div>
          <div>
            <h5 style={{ textAlign: "center" }}>颜色码对照表</h5>
            <table className="table is-fullwidth">
              <thead>
                <tr>
                  <th>
                    <abbr title="Lost">颜色</abbr>
                  </th>
                  <th>
                    <abbr title="Goals for">英文代码</abbr>
                  </th>
                  <th>
                    <abbr title="Goals against">形象描述</abbr>
                  </th>
                  <th>
                    <abbr title="Goal difference">十六进制</abbr>
                  </th>
                  <th>
                    <abbr title="Points">RGB</abbr>
                  </th>
                </tr>
              </thead>
              <tbody id="color">
